import{B as S}from"./core-rjX7nIO3.js";import{b as $,g as v}from"./services-DF6cB9Vv.js";class C extends S{constructor(i){super(i),this.baselineState={selectedBaseline:null,selectedTiers:[],validationResults:null,deploymentPlan:null,groupUsers:{},groupCustomizations:{}},this.userSearchResults=new Map}async render(i,o={}){const c=$.getBaselineConfigurations();i.innerHTML=`
...
This action cannot be undone automatically. Continue?`))return;this.goToStep(4);const o=document.getElementById("baseline-deployment-progress"),c=this.baselineState.deploymentPlan,l=c.groupsToCreate.reduce((n,m)=>n+(m.roles?.length||0),0),e=c.groupsToCreate.length+l+l;let a=0;const s=(n,m=!1)=>{a++;const f=Math.round(a/e*100),h=document.createElement("div");h.className=`deployment-log-entry ${m?"log-error":"log-success"}`,h.innerHTML=`
        ...
        `;try{s("Fetching role information...");const n=await v.getRoleDefinitions(),m=new Map;n.success&&n.roles.forEach(t=>{m.set(t.id,t.displayName)}),s("Starting group creation...");const f=[];for(const t of c.groupsToCreate)try{const r=await v.createPIMGroup(t.displayName,t.description,t.mailNickname);r.success?(f.push({config:t,group:r.group}),s(`✓ Created group: ${t.displayName}`)):s(`✗ Failed to create group: ${t.displayName} - ${r.error}`,!0)}catch(r){s(`✗ Error creating group: ${t.displayName} - ${r.message}`,!0)}s("Waiting for groups to provision (30-60 seconds)..."),await new Promise(t=>setTimeout(t,3e4)),s("Assigning groups to roles...");const h=[];for(const t of f){const r=t.config.roles||[],g=Object.entries(this.baselineState.groupCustomizations||{}).find(([d,u])=>u.name===t.config.displayName),p=g?g[1].assignmentType:"eligible";for(const d of r)try{let u;if(p==="active"?u=await v.createDirectoryRoleActiveAssignment(t.group.id,d,`Baseline deployment (Active): ${t.config.displayName}`):u=await v.createDirectoryRoleEligibilityAssignment(t.group.id,d,`Baseline deployment (Eligible): ${t.config.displayName}`,12),u.success){const b=m.get(d)||d;h.push({groupId:t.group.id,groupName:t.config.displayName,roleId:d,roleName:b,policy:t.config.policy}),s(`✓ Assigned ${t.config.displayName} to ${b} (${p})`)}else s(`✗ Failed to assign ${t.config.displayName}: ${u.error}`,!0)}catch(u){s(`✗ Error assigning ${t.config.displayName}: ${u.message}`,!0)}}if(s("Waiting for role assignments to propagate (10 seconds)..."),await new Promise(t=>setTimeout(t,1e4)),s(`✓ Role assignments complete. ${h.length} groups assigned to roles.`),h.length>0){s("Configuring role policies...");const t=new Map;h.forEach(r=>{t.has(r.roleId)||t.set(r.roleId,{roleId:r.roleId,roleName:r.roleName,groupName:r.groupName,policy:r.policy})});for(const[r,g]of t)try{s(`Fetching policy for ${g.roleName}...`);const p=await v.getRolePolicy(r);if(p.success&&p.policy){let d=0;try{await v.updateRolePolicy(p.policy.id,"Expiration_EndUser_Assignment",{maximumDuration:`PT${g.policy.maximumDurationHours}H`,isExpirationRequired:!0}),d++}catch(u){console.error("Expiration rule update failed:",u)}try{await v.updateRolePolicy(p.policy.id,"Enablement_EndUser_Assignment",{enabledRules:[g.policy.requireMfa?"MultiFactorAuthentication":null,g.policy.requireJustification?"Justification":null,g.policy.requireTicketInfo?"Ticketing":null].filter(u=>u!==null)}),d++}catch(u){console.error("Enablement rule update failed:",u)}try{await v.updateRolePolicy(p.policy.id,"Approval_EndUser_Assignment",{isApprovalRequired:g.policy.requireApproval}),d++}catch(u){console.error("Approval rule update failed:",u)}d>0?s(`✓ Updated ${g.roleName} policy (${d}/3 rules)`):s(`⚠ Could not update ${g.roleName} policy`,!1)}else{const d=p.error||"Unknown error";s(`⚠ Could not fetch ${g.roleName} policy: ${d}`,!1)}}catch(p){s(`⚠ Policy update error: ${p.message}`,!1)}}const y=Object.values(this.baselineState.groupUsers||{}).reduce((t,r)=>t+r.length,0);if(y>0){s(`Adding ${y} users to groups...`);for(const t of f){const r=Object.entries(this.baselineState.groupCustomizations||{}).find(([g,p])=>p.name===t.config.displayName);if(r&&r[1].users&&r[1].users.length>0){const g=r[1].users;for(const p of g)try{const d=await v.createEligibleAssignment(t.group.id,p.id,"member",null,null);d.success?s(`✓ Added ${p.displayName} to ${t.config.displayName}`):s(`✗ Failed to add ${p.displayName}: ${d.error}`,!0)}catch(d){s(`✗ Error adding ${p.displayName}: ${d.message}`,!0)}}}}s("Deployment completed!"),o.innerHTML+=`
...
//# sourceMappingURL=page-baselinepage-B_H2_UCr.js.map
